...
_EXTRACT_SYSTEM = """Du bist ein präziser Kassenbon-Parser. Extrahiere strukturierte Daten
...
- items: Nur erkennbare Einzelposten mit Preis. Leere Liste wenn unklar.
...
def llm_extract(text: str) -> dict[str, Any]:
    """Use small LLM to extract structured receipt fields from OCR text."""
    llm = _state["llm"]
    if llm is None:
    ...
    try:
        ...
        log.warning("LLM extraction failed, falling back to regex: %s", exc)
        return regex_extract(text)
    ...
    # Repair mojibake in string fields
    ...
    if isinstance(data.get("items"), list):
        for item in data["items"]:
            if isinstance(item, dict) and isinstance(item.get("name"), str):
                item["name"] = _repair_mojibake(item["name"])
    ...
    if isinstance(raw_items, list):
        for item in raw_items:
            ...
            if isinstance(name, str) and name.strip() and isinstance(item_amount, (int, float)):
                items.append({"name": name.strip(), "amount": round(float(item_amount), 2)})
...
async def extract_receipt(file: UploadFile = File(...)) -> ReceiptResult:
    ...
    def _pipeline(data: bytes) -> dict[str, Any]:
        ...
        if REGEX_ONLY or _state["llm"] is None:
            fields = regex_extract(full_text)
        else:
            fields = llm_extract(full_text)

        return {**fields, "raw_text": full_text, "ocr_confidence": round(avg_confidence, 3)}

    result = await _run_blocking(_pipeline, img_bytes)
    elapsed_ms = int((time.monotonic() - t0) * 1000)
    result["processing_ms"] = elapsed_ms
    log.info(
        "Extracted receipt: amount=%s date=%s store=%s confidence=%.2f time=%dms",

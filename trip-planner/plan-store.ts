 ...
 *
 * The shape mirrors what the solver produces — days, blocks, stops —
 * plus the pool, which is the working set redistribution turns on
 * rather than a bin for leftovers (§5).
 *
...
export async function createPlan(input: CreatePlanInput, db: Db = dbDefault): Promise<number> {
  const [plan] = await db
    ...
    .values({
      ...
      anchor_lon: input.anchor.lon,
      region_db: input.regionDb,
      constraints: input.constraints,
    })
    .returning({ id: tripPlans.id });

  for (const [dayIndex, blocks] of input.days.entries()) {
    const [day] = await db
      .insert(tripPlanDays)
      .values({ plan_id: plan.id, day_index: dayIndex })
...
export async function loadPlan(
  ...
  const dayRows = await db
    .select()
    .from(tripPlanDays)
    .where(eq(tripPlanDays.plan_id, planId))
    .orderBy(asc(tripPlanDays.day_index));
  ...
  const poolRows = await db
    .select()
    .from(tripPlanPool)
    .where(eq(tripPlanPool.plan_id, planId));
  ...
  for (const row of stopRows) {
    ...
    list.push({
      ...
      travelFromPrevious: {
        ...
        travelClass: row.travel_minutes < 10 ? "short_walk" : "long_walk",
      },
  ...
  return {
    ...
    title: plan.title,
    anchor: { lat: plan.anchor_lat, lon: plan.anchor_lon },
    regionDb: plan.region_db,
...
/**
 * Replace one day's blocks and the plan's pool with the result of a
...
export async function saveRedistribution(
  ...
  await db.delete(tripPlanPool).where(eq(tripPlanPool.plan_id, planId));
  if (pool.length > 0) {
    await db.insert(tripPlanPool).values(
      pool.map((c) => ({
        plan_id: planId,

...
export const userSubjectPersons = pgTable("user_subject_persons", {
  ...
  relation_kind: text("relation_kind")
    .$type<"self" | "spouse" | "child" | "parent" | "sibling" | "ward" | "other">()
    .notNull()
    .default("other"),
  birth_date: text("birth_date"),
  in_household: boolean("in_household").notNull().default(false),
  tax_cost_bearer: text("tax_cost_bearer")
    .$type<"user" | "person" | "unknown">()
    .notNull()
    .default("unknown"),
...
export const userAssessmentSettings = pgTable("user_assessment_settings", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  assessment_type: text("assessment_type")
    .$type<"zusammen" | "einzeln" | "unknown">()
    .notNull()
    .default("unknown"),
  valid_from_tax_year: integer("valid_from_tax_year"),
  created_at: timestamp("created_at", { mode: "string" }).notNull().defaultNow(),
  updated_at: timestamp("updated_at", { mode: "string" }).notNull().defaultNow(),
});
...
export const osmRegionImports = pgTable("osm_region_imports", {
  ...
  replication_last_attempt_at: timestamp("replication_last_attempt_at", { withTimezone: true, mode: "string" }),
  replication_last_success_at: timestamp("replication_last_success_at", { withTimezone: true, mode: "string" }),
  replication_failure_count: integer("replication_failure_count").notNull().default(0),
  replication_next_retry_at: timestamp("replication_next_retry_at", { withTimezone: true, mode: "string" }),
  last_error: text("last_error"),
  created_at: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
  updated_at: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
});

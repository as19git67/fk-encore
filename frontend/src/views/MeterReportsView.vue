...
async function load() {
  ...
  error.value = ''
  try {
    // The meter list is not shown here — it only answers whether there is
    // anything to report on yet, so the empty page can say what to do first.
    meters.value = (await listMeters()).meters
    await Promise.all([
      loadEnergyReport(),
      loadTrends(),
      loadEconomics(),
    ...
    error.value = err.message || 'Fehler beim Laden der Auswertungen'
  } finally {
    loading.value = false
  }
}
...
<template>
  <PageLayout title="Auswertungen" width="normal" :ready="!loading">
    ...
    <div v-else-if="meters.length === 0" class="info">
      Noch keine Zähler angelegt — ohne Ablesungen gibt es nichts auszuwerten.
      <Button label="Zur Zählerverwaltung" icon="pi pi-arrow-right" link @click="router.push({ name: 'zaehler-list' })" />
    </div>

...
test.describe('Foto-Galerie (Maus-fokus)', () => {
  ...
  test('Klick auf Auswählen → Shift-Klick selektiert ersten und dritten Eintrag', async ({ page }) => {
    ...
    // The compact tray also offers "Auswahl beenden". Assert the toolbar's
    // own control specifically, rather than relying on an accessible-name
    // match that is intentionally shared by both exit controls. Since
    // stage 3 (#1278) that control is the shared list toolbar's.

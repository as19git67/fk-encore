...
describe("normalizeForMatch", () => {
  it("lowercases and strips spaces/punctuation, keeps umlauts and digits", () => {
    expect(normalizeForMatch("Open Text Software GmbH")).toBe("opentextsoftwaregmbh");
...
describe("matchSenderRule", () => {
  ...
  it("disambiguates a forwarded tax assessment from the same employer (rule order)", () => {
    expect(
      matchSenderRule({
        sender: "Open Text Software GmbH",
  ...
  it("honours requireAny for municipal fees", () => {
    expect(
      matchSenderRule({ sender: "Gemeinde Merching", title: "Bescheid Wasser und Abwasser 2023" }),
    ).toBe("wohnen-kommunale-abgaben");
    expect(
      matchSenderRule({ sender: "Gemeinde Merching", title: "Einladung Bürgerversammlung" }),
    ).toBeNull();
  });
});
